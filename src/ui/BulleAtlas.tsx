...
/* ---------------------------------------------------------------
   ...
   Ce fichier ne s'occupe plus que du BOUTON : le regard, le
   déplacement, les bords qu'il n'a pas le droit de franchir. La
   conversation elle-même vit dans `Causerie.tsx` — elle est devenue
   un écran à part entière, avec une trentaine de logiques derrière.

   Honnêteté du contenu : l'IA n'existe pas encore (elle arrive en
   V1). Atlas ne fait donc pas semblant de comprendre — il exécute
   des règles nommées, ou il dit qu'il ne comprend pas. Une fausse
   compréhension coûterait plus cher que l'absence de réponse.
   --------------------------------------------------------------- */
...
export function BulleAtlas() {
  ...
  return (
    <>
      ...
      {/* MONTÉE ET DÉMONTÉE, jamais simplement masquée : le briefing
          se recalcule à chaque ouverture. Une conversation qui garde
          son fil d'il y a trois jours annoncerait « 12 notes à
          ranger » alors qu'elles sont rangées depuis. */}
      {ouverte && <Causerie fermer={() => setOuverte(false)} />}
    </>
  )
}
